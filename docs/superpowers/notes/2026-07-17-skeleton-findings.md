...
### 真機發現(2026-07-21):設計輸入 A 單發測試——3/3 全過,佐證競態只在連發時觸發

單發測試(B1/B2/B3,間隔 15~30 秒,全程未碰人工輸入框)。以 outbox 產物逐一核對:
- taster 3/3 正確判定(B1/B2/B3 各自獨立、內容正確)。
- cyrano 3/3 正確回覆,內容各自對題、**無重複/誤送**。

**結論**:上一則連發測試(A1~A5)發現的 Stop hook 讀檔競態(A4 誤送 A3 內容)**只在訊息間隔壓縮到幾秒內時才會觸發**;間隔拉開到 15 秒以上完全正常。佐證根因確實是「CLI 生成完成到 transcript 落地磁碟」與「下一輪注入+新一輪 Stop 觸發」之間的時間差過短所致——連發情境下,上一輪的磁碟寫入可能還沒完成,下一輪的 Stop 事件就已經觸發 hook 讀檔。
